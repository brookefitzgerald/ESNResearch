...
teacher = np.array([np.sin(2*np.pi*n/10) for n in np.arange(0,nTeachRuns-1)])
...
X[:,0]=1.0
Y = np.zeros(nTeachRuns)
Y[1:(nTeachRuns)]=teacher

for i in np.arange(2, nTeachRuns):
	W_dot_Xn = np.dot(W,X[:,i-1].reshape(100,1))
	Win_dot_Yn = np.dot(W_in,Y[i-1])
...
Y_dot_Xt = np.dot(Y.reshape(1,nTeachRuns),X.T)
...
training_MSE = np.sqrt(np.sum((training_results[100:nTeachRuns]-Y[100:nTeachRuns])**2))/(nTeachRuns-100)

plt.plot(training_results[100:nTeachRuns],Y[100:nTeachRuns])
...
for i in np.arange(2,nTestRuns):
